...
reviews_pad = np.zeros((len(reviews_num), 1500), dtype = int)

for i, review in enumerate(reviews_num):
    review_len = len(review)

    if review_len <= 1500:
        zeroes = list(np.zeros(1500-review_len))
        new = zeroes+review
    elif review_len > 1500:
        new = review[0:1500]
...
class ConvNet(nn.Module):
    def __init__(self, num_classes=10):
        ...
        self.layer1 = nn.Sequential(
            nn.Conv2d(in_channels = 1, out_channels = 16, kernel_size=(3,1), stride=1, padding=1),
            nn.BatchNorm2d(16),
        ...
        self.layer2 = nn.Sequential(
            nn.Conv2d(in_channels = 32, out_channels = 64, kernel_size=(3,1), stride=1, padding=1),
            nn.BatchNorm2d(64),
            nn.Conv2d(in_channels = 64, out_channels = 128, kernel_size=(3,1), stride=1, padding=1),
            nn.BatchNorm2d(128),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=(2,1), stride=2),)
        self.layer3 = nn.Sequential(
            nn.Conv2d(in_channels = 128, out_channels = 256, kernel_size=(3,1), stride=1, padding=1),
            nn.BatchNorm2d(256),
            nn.Conv2d(in_channels = 256, out_channels = 512, kernel_size=(3,1), stride=1, padding=1),
            nn.BatchNorm2d(512),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=(5,1), stride=5),)
        self.layer4 = nn.Sequential(
            nn.Conv2d(in_channels = 512, out_channels = 1024, kernel_size=(3,1), stride=1, padding=1),
            nn.BatchNorm2d(1024),
            nn.Conv2d(in_channels = 1024, out_channels = 1024, kernel_size=(3,1), stride=1, padding=1),
            nn.BatchNorm2d(1024),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=(5,1), stride=5),)
        ...
        self.fc = nn.Sequential(
            nn.Dropout(p = 0.5),
            nn.Linear(in_features = 6144, out_features = 2048),
